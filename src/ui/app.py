...
def run_app():
    ...
    if menu == "캡처 및 저장":
        ...
        for i in range(capture_count):
            if st.button(f"캡처 이미지 {i+1} 촬영", key=f"capture_button_{i}"):  # 고유한 key 설정
                images = capture_images_from_webcam(target_count=1, key=f"capture_button_{i}")
                ...
                if images:
                    ...
                    if landmarks:
                        ...
                        st.success(f"이미지 {i+1}가 성공적으로 캡처되었습니다.")
                    else:
                        st.error(f"이미지 {i+1}에서 얼굴 랜드마크를 추출할 수 없습니다.")
                else:
